...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Ne pas appliquer les correctifs iOS sur Android
  if (isAndroid) return;
  ...
  if (isIOS) {
    ...
    const touchElements = document.querySelectorAll(
      '[data-haptic="true"], .btn, .btn-oxygen, .btn-primary, .btn-outline, a, button'
    );
    ...
    const fixedElements = document.querySelectorAll(
      "header, .fixed-element, .contrast-controls, .swipe-indicator"
    );
    ...
    if (window.swipeNavigation) {
      ...
      const originalNavigateToSection =
        window.swipeNavigation.navigateToSection;
