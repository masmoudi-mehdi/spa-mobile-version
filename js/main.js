    ...
    if (!isMobile) {
        ...
        window.addEventListener('scroll', function() {
            ...
            if (scrollPosition < heroHeight) {
                ...
                heroImage.style.transform = `scale(${1.05 + scrollPercentage * 0.05}) translateY(${scrollPosition * 0.15}px)`;
    ...
    window.addEventListener('scroll', function() {
        if (!scrollTimeout) {
            scrollTimeout = setTimeout(function() {
                scrollTimeout = null;
                checkIfInView();
            }, isMobile ? 100 : 50); // Longer timeout on mobile for better performance
        }
    });
